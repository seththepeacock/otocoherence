...
for wf_fn in wf_fns:
    ...
    plt.tight_layout()
    if output_plots:
        fig_fp = os.path.join(dirs["Human Peak Picks (Fig.4)"], f"{wf_fn} [{method_type}].jpg")
    ...
    plt.tight_layout()
    if output_plots:
        fig_fp = os.path.join(dirs["Human Peak Picks (Fig.4)"], "C_xi_phi Plots", f"{wf_fn} [{method_type}].jpg")
        plt.savefig(fig_fp, dpi=500)
...
if output_spreadsheet:
    ...
    ss_path = os.path.join(dirs["Human Peak Picks (Fig.4)"], spreadsheet_fn)
