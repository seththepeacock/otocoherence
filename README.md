...
## Scripts
All analysis code is in `/scripts`. `soae_N_xi.py` will analyze all SOAE waveforms using two windowing methods and generate `.pkl` files of the analysis in `/pickles` for future use in plots; `nddho_N_xi.py` will simulate noise driven damped harmonic oscillator waveforms (using the function in `nddho_generator.py`) and analyze them similarly. `figures_main.ipynb` (and `figures_SI.ipynb`) will generate all figures used in [the manuscript](https://www.biorxiv.org/content/10.1101/2025.11.14.687084v1).
